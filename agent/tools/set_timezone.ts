...
import { setUserTimezoneFromSource } from "@/lib/user-prefs";
...
// so it stops asking once it knows. Source "user_stated" is the
// highest-ranked source in lib/user-prefs.ts's precedence rule, so this
// always overwrites - a value the user just stated in chat outranks anything
// already on file, including a Google Calendar sync or a low-confidence
// guess.
export default defineTool({
  ...
  async execute({ timezone }, ctx) {
    ...
    await setUserTimezoneFromSource(owner.workspaceId, timezone, "user_stated");
